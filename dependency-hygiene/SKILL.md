...
- Upstream moved (new releases, a teammate bumped the catalog, a peer changed) → reconcile: an unpushed safe bump **supersedes to the newer latest-allowed** (re-derive, diff-skip if identical), fresh drift folds in, open decisions re-present.
- At latest-allowed *as of this run*, gates green, no open decisions → closed for this run. Drift published later is the next invocation's work, not a reopening.

## Run – in Order

1. **Discover the repo's dependency practice.** The version source (single `package.json`? a pnpm `catalog`? workspace protocol?), lockfile + package manager, `overrides`, `patchedDependencies`, version/trust policies (`trustPolicy`, `minimumReleaseAge`, and any `trustPolicyExclude`/`minimumReleaseAgeExclude` – read these as **pre-vetted**, never re-surfaced as decisions), any renovate/dependabot config, pinning conventions – and the host for the eventual MR (GitLab → `glab`, GitHub → `gh`). The maturity/trust policies feed taze in step 2. Done once each is named, discovered fresh for this repo.
...
[ ] push commits   [ ] open MR [show description]   [ ] post tracker / ticket [show]
```

## After Approval

Safe order for this tray: push commits → open the MR → post the tracker update. The execution-time re-check carries one extra condition here – a clean `install` plus the gates still pass on the current base, since a lockfile rots if the catalog moved under it since the brief. A workspace that no longer installs green aborts the push.
