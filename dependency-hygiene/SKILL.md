...
10. **Pre-draft outward copy via `/write-dev-copy` – stage, never post.** The MR description, an optional comment, an optional tracker update – or, when a great breaking change is deferred, a follow-up ticket capturing the migration scope. Language matches the audience – the repo's working language.
