...
Discover what's drifted, sweep the safe bumps, and land the deliberate ones cleanly across the workspace – conform to the repo's own dependency practice, migrate breaking changes against the installed source, get every gate green – then stop once, late, with a grill-ready brief so the human resolves only the real judgment calls and picks what ships.

Dependency work here is manual – no renovate, no dependabot – and the judgment sprawls: the ESLint→antfu migration was almost entirely "which rules to keep, drop, enable." This loop collapses that grind into one brief on a workspace that already compiles.
...
9. **Verify the full-workspace gates** (discovered): clean install, lint, typecheck, build, tests. The lockfile's blast radius is the whole workspace, so the gates run **unscoped** – and the workspace was green before, so any red is the change's doing. A safe-batch bump that turns a gate red **drops out of the batch into a Decision** rather than shipping red; a deep landing's red → fix it, or recommend pin/defer. Nothing is "done" until they pass; a gate that genuinely can't run (secrets, env) is named in the brief, never a silent "should work."
...
- **A choice** – an API split into two replacements where which is right depends on intent → apply the **conservative, behavior-preserving** option to keep the workspace compiling, and surface the choice as a Decision (the path taken, the alternative, a recommendation).
...
Change + practice first; the safe batch is **Applied**, not a Decision; Decisions holds only judgment calls, with the majors the sweep left untouched as a single awareness line; Applied is FYI, one change per commit; verification is explicit; the tray closes. Drafts collapse behind `[show]`. `write-dev-copy` voice throughout (en dash, backticked identifiers, fact-only).
...
- Open the MR – drafted description.
- Post the optional comment / Asana update, or the deferred-migration ticket – only if drafted.
...
- **DOES NOT adopt** optional rules or changed defaults autonomously – those are decisions.
- Does not force-align deliberate divergence (pins, overrides, patches) – it flags them.
- Does not hardcode a dependency practice – catalog, trust policy, maturity, pinning are discovered per repo.
- Does not use taze `-I`/`-i`/`-u`, nor `pnpm up --latest`/`ncu` – they corrupt catalog refs.
- Does not add code comments that restate implementation.

## Degradation

Discovery can't reach the registry (offline, registry down), a gate can't run (needs secrets or env), a breaking change has no migration guide, a major's blast radius outweighs the change → the brief **states it plainly**, presents what compiled, and recommends pin/defer rather than forcing a half-migrated workspace through. Never claim a green that wasn't observed.
